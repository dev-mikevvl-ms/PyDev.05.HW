...
def tRefillAcc_ffp(laSf_o, file=sys.stdout):
  ...
  tOutStt_d['kAccSum_n'] += loAdd_n #DVL: input by inp_FltAVali_fefi
  # print(f'DBG: На счету:({tOutStt_d['kAccSum_n']:.2f}) и в истории покупок {len(tOutStt_d['kBuyHstT_l'])} зап.')
  print(f'Пополнение на:({loAdd_n:.2f}).', file=file)
  return loAdd_n

def tBuy_ffp(laSf_o, file=sys.stdout):
  if tOutStt_d['kAccSum_n'] <= 0:
    print(f"На Вашем счету:({tOutStt_d['kAccSum_n']:.2f}) <= 0.",
        ' Пополните счет, пожалуйста.', sep='\n', file=file)
    return (None, None)

  loCost_n = inp_FltAVali_fefi(f" сумму покупки (на Вашем счету:{tOutStt_d['kAccSum_n']:.2f})\n",
      laInPTypeFlt_cll=float, laDfV_s=f"{min(100.00, tOutStt_d['kAccSum_n']):.2f}",
      laAcceptEmptyInPAsDf_b=True, laValiInPMsg_s=f'положительное число с возм.десят.точкой\n',
      laVali_cll=lambda _n: 0 <= _n, file=file)[0]

  if tOutStt_d['kAccSum_n'] < loCost_n: #DVL: input by inp_FltAVali_fefi
    print(f"Денег на Вашем счету:({tOutStt_d['kAccSum_n']:.2f}) не хватает",
  ...
  # print(f'DBG: На счету:({tOutStt_d['kAccSum_n']}) и в истории покупок {len(tOutStt_d['kBuyHstT_l'])} зап.')
  tOutStt_d['kAccSum_n'] -= loCost_n
  tOutStt_d['kBuyHstT_l'].append((loDesc_s, loCost_n)) #DVL: input by inp_FltAVali_fefi
  # print(f'DBG: На счету:({tOutStt_d['kAccSum_n']}) и в истории покупок {len(tOutStt_d['kBuyHstT_l'])} зап.')
  print(f'Покупка: "{loDesc_s}", на сумму:({loCost_n:.2f}).', file=file)
  return (loDesc_s, loCost_n)

def tVieHst_ffp(laSf_o, file=sys.stdout):
  print(f"История покупок (всего {len(tOutStt_d['kBuyHstT_l'])} зап.):",
      *enumerate(tOutStt_d['kBuyHstT_l'], 1), '', sep='\n', file=file)
  return (tOutStt_d['kAccSum_n'], len(tOutStt_d['kBuyHstT_l']))
...
def prn_OutStt_fp(laSf_o, file=sys.stdout):
  # 2Do: CheExs(kAccSum_n, kBuyHstT_l)
  if 'kAccSum_n' in tOutStt_d and 'kBuyHstT_l' in tOutStt_d:
    print(f"На счету:({tOutStt_d['kAccSum_n']:.2f})",
        f"и в истории покупок {len(tOutStt_d['kBuyHstT_l'])} зап.",
...
def main(laArgs: list[str], *laArg_l, **laKwArg_d) -> dict:
  ...
  #     fHeaFmt_s= glSep_s + f'\n{loAppDesc_s}:'))
  loKwArg_d.update(dict(fPrnOutStt_cll=prn_OutStt_fp,
      fHeaFmt_s= glSep_s + f'\n{loAppDesc_s}:'))
