...
def tRefillAcc_ffmp(self, file=sys.stdout):
  ...
  self.fInnStt_d['kAccSum_n'] += loAdd_n #DVL: input by inp_FltAVali_fefi
  # print(f'DBG: На счету:({self.fInnStt_d['kAccSum_n']:.2f}) и в истории покупок {len(self.fInnStt_d['kBuyHstT_l'])} зап.')
  print(f'Пополнение на:({loAdd_n:.2f}).', file=file)
  return loAdd_n

def tBuy_ffmp(self, file=sys.stdout):
  if self.fInnStt_d['kAccSum_n'] <= 0:
    print(f"На Вашем счету:({self.fInnStt_d['kAccSum_n']:.2f}) <= 0.",
        ' Пополните счет, пожалуйста.', sep='\n', file=file)
    return (None, None)

  loCost_n = inp_FltAVali_fefi(f" сумму покупки (на Вашем счету:{self.fInnStt_d['kAccSum_n']:.2f})\n",
      laInPTypeFlt_cll=float, laDfV_s=f"{min(100.00, self.fInnStt_d['kAccSum_n']):.2f}",
      laAcceptEmptyInPAsDf_b=True, laValiInPMsg_s=f'положительное число с возм.десят.точкой\n',
      laVali_cll=lambda _n: 0 <= _n, file=file)[0]

  if self.fInnStt_d['kAccSum_n'] < loCost_n: #DVL: input by inp_FltAVali_fefi
    print(f"Денег на Вашем счету:({self.fInnStt_d['kAccSum_n']:.2f}) не хватает",
  ...
  # print(f'DBG: На счету:({self.fInnStt_d['kAccSum_n']}) и в истории покупок {len(self.fInnStt_d['kBuyHstT_l'])} зап.')
  self.fInnStt_d['kAccSum_n'] -= loCost_n
  self.fInnStt_d['kBuyHstT_l'].append((loDesc_s, loCost_n)) #DVL: input by inp_FltAVali_fefi
  # print(f'DBG: На счету:({self.fInnStt_d['kAccSum_n']}) и в истории покупок {len(self.fInnStt_d['kBuyHstT_l'])} зап.')
  print(f'Покупка: "{loDesc_s}", на сумму:({loCost_n:.2f}).', file=file)
  return (loDesc_s, loCost_n)

def tVieHst_ffmp(self, file=sys.stdout):
  print(f"История покупок (всего {len(self.fInnStt_d['kBuyHstT_l'])} зап.):",
      *enumerate(self.fInnStt_d['kBuyHstT_l'], 1), '', sep='\n', file=file)
  return (self.fInnStt_d['kAccSum_n'], len(self.fInnStt_d['kBuyHstT_l']))

def tExit_fm(self, file=sys.stdout):
  self.fIsRun_b = False

# tMenu_d = {'1':('Пополнение счета', tRefillAcc_ffmp, ??Type??(Exit, Back, SbMenu, CtrlVieMenu??)),
#     '2':('Покупка', tBuy_fm),
...
def main(laArgs: list[str]) -> None:
  # Ww(sys.argv[1:])
  tMenu_o = Menu_c({1:('Пополнение счета', tRefillAcc_ffmp),
    '2':('Покупка', tBuy_ffmp),
    '3':('История покупок', tVieHst_ffmp),
    # 'E':('Выход', tExit_fm),
    '4':('Выход', tExit_fm)
    }, fInnStt_d=tInnStt_d, fPrnInnStt_cll=prn_InnStt_fp,
    fHeaFmt_s= glSep_s + '\nМой банковский счет:')
