...
def mA_SysInfo_ffmp(laSf_o, file=sys.stdout):
  loMsg_s = f"{sys.platform=}\n{sys.version=}\n{sys.api_version=}."
  print(loMsg_s, file=file)
  return loMsg_s
def mA_MyInfo_ffmp(laSf_o, file=sys.stdout):
  loMsg_s = "Creator/Author: Mike Vl. Vlasov <dev.mikevvl@outlook.com>."
  print(loMsg_s, file=file)
  return loMsg_s

def mA_victory_ffmp(laSf_o, file=sys.stdout):
  return mod.victory.main(None)
def mA_MyBankAcc_ffmp(laSf_o, file=sys.stdout):
...
def main(laArgs: list[str], *laArg_l, **laKwArg_d) -> dict:
  ...
  loMenu_o = mod.MVVlStd.Menu_c(mMenu_d, **loKwArg_d)
